...
from src.utils.models import TLTumorClassifier
...
if params["data"]["color_mode"] == "rgb":
    COLOR_CHANNELS = 3
elif params["data"]["color_mode"] == "greyscale":
    COLOR_CHANNELS = 1
...
logging.basicConfig(
    ...
    handlers = [
        ...
        logging.FileHandler(f"logs/transfer_learning_model_general_logs_{datetime.date.today().strftime('%Y-%m-%d')}.log")
    ]
)
...
csv_logger = CSVLogger(
    filename = f"logs/transfer_learning_model_training_logs_{datetime.date.today().strftime('%Y-%m-%d')}.csv"
)
...
model = TLTumorClassifier(
    image_height = params["data"]["image_height"],
    image_width = params["data"]["image_width"],
    image_channels = COLOR_CHANNELS,
    base_model = params["model"]["transfer_learning"]["base_model"],
    pool_type = params["model"]["transfer_learning"]["pool_type"],
    dense_layer_units = params["model"]["transfer_learning"]["dense_units"],
    dropout_pct = params["model"]["transfer_learning"]["dropout_pct"]
)
...
model.save(params["model"]["transfer_learning"]["saved_model_dir"]) # save the model if desired
logging.info(f"Model architecturs and weights saved to {params["model"]["transfer_learning"]["saved_model_dir"]}")